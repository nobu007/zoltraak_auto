...
class MagicInfo(BaseModel):
    ...
    target_file_path: str = Field(default="", description="処理対象のファイルパス、空白は検索モード")
    target_dir: str = Field(default="./output", description="出力先のディレクトリ、デフォルトは")
...
def display_magic_info_pre(magic_info: MagicInfo):
    ...
    table.add_row("魔法術式 (要件定義書)", magic_info.target_file_path)
...
def display_magic_info_post(magic_info: MagicInfo):
    ...
    table.add_row("魔法術式 (要件定義書)", magic_info.target_file_path)
    table.add_row("領域", magic_info.target_dir)
