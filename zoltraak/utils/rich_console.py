...
from zoltraak.llms.litellm_api import LitellmParams
...
def display_magic_info_init(magic_info: MagicInfo):
    ...
    table.add_row("神託　　 (エターナルインテント.md)", file_info.destiny_file_path)
    ...
    table.add_row("古代魔導書 (高級言語プログラム.py)", file_info.py_file_path)
...
def display_magic_info_post(magic_info: MagicInfo):
    ...
    _add_row_relpath(table, "錬成器(compiler)", magic_info.grimoire_compiler, magic_info.file_info.work_dir)
    _add_row_relpath(table, "起動式(architect)", magic_info.grimoire_architect, magic_info.file_info.work_dir)
    _add_row_relpath(table, "調律石(formatter)", magic_info.grimoire_formatter, magic_info.file_info.work_dir)
...
def generate_response_with_spinner(
    ...
    if magic_info.is_async:
        # 非同期モードではスピナーを表示しない
        result = generate_response_fn(litellm_params)
    else:
