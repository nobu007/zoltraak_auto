...
| 名称 | 魔法体系 | 自然言語プログラミング体系 |
|:--|:--|:--|
| 絶対空間 | 領域術式を展開する空間そのもの | 出力先のルートディレクトリ |
| 領域名 | 絶対空間の中に展開された領域の名前 | canonical_name |
| 領域情報 | 領域術式が展開された場所（絶対空間のどこか？） | target_dir |
| 魔導書名 | 領域展開で最後に使われた魔導書の名前 | current_grimoire_name |
| 錬成器 | 展開に使われた錬成器の名前 | grimoire_compiler |
| 起動式 | 錬成器を起動する術式の名前 | grimoire_architect |
| 調律石 | 錬成器による変換の精度と安定性を高める調律石の名前 | grimoire_formatter |
| 魔法術式 (錬成前) | 錬成器が変換する前の魔法術式 | source_file_path |
| 魔法術式 (錬成後) | 錬成器が変換した後の魔法術式 | source_file_path |
