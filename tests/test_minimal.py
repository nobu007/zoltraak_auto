...
class MinimumTestCase(unittest.TestCase):
    def test_zoltraak_only(self):
        ...
        self.assertIn(
            "エラー: 入力ファイルまたはテキストが指定されていません。", result.stdout
        )  # エラーメッセージは標準エラー出力に含まれているはずなので、result.stderrをチェックするように修正

    # def test_zoltraak_with_compiler(self):
    #     """zoltraak "〜したい" -cコマンドのテスト"""
    #     result = SubprocessUtil.run(["zoltraak", "テストしたい", "-c", "コンパイラ"], capture_output=True, text=True)
    #     self.assertEqual(result.returncode, 0)
    #     self.assertIn("コンパイル結果", result.stdout)

    # def test_zoltraak_without_compiler(self):
    #     """zoltraak "〜したい" -cコマンド（コンパイラなし）のテスト"""
    #     result = SubprocessUtil.run(["zoltraak", "テストしたい", "-c"], capture_output=True, text=True)
    ...
    #     result = SubprocessUtil.run(["zoltraak", "テストしたい", "-cc", "自作コンパイラ"], capture_output=True, text=True)
    #     self.assertEqual(result.returncode, 0)
